...
- To run attacks with the baselines:
    - `python manager.py --task attack --kind bs_onepixel --data imagenet --model googlenet --model_attr vgg`
        > You may use any of the models from above here; and `bs_square` or `bs_pixle`. We set `model_attr` argument above to skip the images for which the `vgg` model fails.
...
- Run `clear && python manager.py --task attack_llava --kind attr --data imagenet --model vgg --model_attr vgg --opt_d 2500 --opt_m 1000 --opt_sc 0.6 --opt_k 50 --opt_k_top 5 --opt_k_gd 10 --opt_lr 1.E-2 --llava_prompt "Briefly describe what a person might do with the main object depicted in the image." --attack_num_max 100`
